...
请将以下内容添加到 `Info.plist` 顶层 `<dict>` 中：
...
`EngineError` 是一个公开枚举，由 `play(...)`、`applyMix(...)` 等 `async throws` 接口抛出。常见类型包括：

- `.invalidURL` — 提供的音频 URL 无法解析
- `.maxTracksExceeded` — 已达到配置的 `maxConcurrentTracks` 上限
- `.networkFailure(Error)` — 下载失败
- `.cacheError(Error)` — 磁盘缓存读写失败

建议使用 `do/catch` 包裹调用，以便在 UI 中进行友好提示。
...
欢迎提交 Issue 和 Pull Request。如涉及较大改动，建议先开 Issue 讨论。

## 许可证

请在此处填写所采用的许可证（如 MIT、Apache 2.0 等）。